...
from transformers import BertConfig, AutoTokenizer, AutoConfig, AutoModel
from bert_model import HierarchicalLSTMBertCRF
import utils
import copy
...
def parse_arguments_t(parser):
    ...
    # 添加FinBERT相关参数
    parser.add_argument('--bert_model_name', type=str, default="ProsusAI/finbert",
                        help="Name or path of the FinBERT model")
    parser.add_argument('--freeze_bert_layers', type=int, default=0,
                        help="Number of BERT layers to freeze (0 means no freezing)")
...
def train_model(config: Config, train_insts: List[List[Instance]], dev_insts: List[Instance]):
    ...
    for iter in range(num_outer_iterations):
        ...
        for fold_id, folded_train_insts in enumerate(train_insts):
            ...
            model_name = model_folder + f"/bert_crf_{fold_id}"
            model = HierarchicalLSTMBertCRF(cfig=cfig)
            ...
            hard_constraint_predict(config=config, model=model,
                                    ...
                                    folded_insts=train_insts[
                                        1 - fold_id])  # set a new label id, k is set to 2, so 1 - fold_id can be used
        logging.info("\n\n")
...
def hard_constraint_predict(config: Config, model: HierarchicalLSTMBertCRF, fold_batches: List[Tuple],
                            folded_insts: List[Instance], model_type: str = "hard"):
    ...
    for batch in tqdm(fold_batches):
        ...
        batch_max_scores, batch_max_ids = model(input_ids, input_seq_lens=input_seq_lens,
                                                annotation_mask=annotation_mask, labels=None,
...
def train_one(config: Config, train_batches: List[Tuple], dev_insts: List[Instance], dev_batches: List[Tuple],
              model_name: str, config_name: str = None) -> HierarchicalLSTMBertCRF:
    # 加载FinBERT配置
    finbert_config = AutoConfig.from_pretrained(config.bert_model_dir)
    finbert_config.device = config.device
    finbert_config.label2idx = config.label2idx
    finbert_config.label_size = config.label_size
    finbert_config.idx2labels = config.idx2labels
    
    # 添加额外的配置
    finbert_config.freeze_bert_layers = opt.freeze_bert_layers
    
    # 初始化模型
    model = HierarchicalLSTMBertCRF(finbert_config)
    
    # 加载预训练权重
    pretrained_bert = AutoModel.from_pretrained(config.bert_model_dir)
    ...
    bilstm_params = list(model.bilstm.named_parameters())
    cnn_params = list(chain(*[conv.named_parameters() for conv in model.conv_layers]))
    other_params = list(chain(
        model.attention.named_parameters(),
        model.feature_fusion.named_parameters(),
    ...
    optimizer_grouped_parameters = [
        ...
        # BiLSTM参数
        {
            'params': [p for n, p in bilstm_params],
            'lr': config.learning_rate,
            'weight_decay': 0.01
        },
        # CNN参数
        {
            'params': [p for n, p in cnn_params],
            'lr': config.learning_rate,
            'weight_decay': 0.01
        },
        # 其他参数
    ...
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
        optimizer,
        mode='max',           # 因为我们要最大化F1分数
        factor=0.5,          # 当性能不再提升时，将学习率减半
        patience=2,          # 等待2个epoch验证集性能无提升后再调整
        verbose=True,        # 打印学习率变化信息
        min_lr=1e-6         # 学习率下限
    )

    model.train()  # 确保模型在训练开始时处于训练模式
    ...
    for i in range(1, epoch + 1):
        ...
        for index in tqdm(np.random.permutation(len(train_batches))):
            ...
            if not model.training:
                print(f"Warning: Model was in eval mode during training at batch {index}")
                model.train()

            # 获取batch数据
            input_ids, input_seq_lens, annotation_mask, labels = train_batches[index]

            # 生成attention_mask并确保所有数据在正确的设备上
            attention_mask = (input_ids != 0).bool().to(config.device)

            # 数据移动到设备
            input_ids = input_ids.to(config.device)
            input_seq_lens = input_seq_lens.to(config.device)
            ...
            try:
                ...
                if isinstance(loss, tuple):
                    print(f"Warning: Loss is tuple with length {len(loss)}")
                    # 如果是tuple，取第一个元素（通常是loss值）
                    loss = loss[0]

                if not isinstance(loss, torch.Tensor):
                    print(f"Warning: Loss is not a tensor, type: {type(loss)}")
                    continue

                if loss.dim() > 0:
                    print(f"Warning: Loss has dimensions {loss.shape}, taking mean")
                    loss = loss.mean()
                ...
                print(f"Debugg：Error in training step: {str(e)}")
                print(f"Current batch index: {index}")
                print(f"Loss type: {type(loss) if 'loss' in locals() else 'Not computed'}")
                if 'loss' in locals() and isinstance(loss, tuple):
                    print(f"Loss tuple contents: {loss}")
                continue
        ...
        with torch.no_grad():
            ...
            for idx, param_group in enumerate(optimizer.param_groups):
                logging.info(f"Group {idx} - Current learning rate: {param_group['lr']}")
        ...
        logging.info(f"Dev F1: {current_f1:.4f}, Best Dev F1: {best_dev_f1:.4f}")

        # 确保在评估后显式切换回训练模式
        model.train()
        print(f"End of epoch {i}, model mode: {'train' if model.training else 'eval'}")

    return model


def evaluate_model(config: Config, model: HierarchicalLSTMBertCRF, batch_insts_ids, name: str, insts: List[Instance]):
    # evaluation
...
def evaluate_model_for_entity(config: Config, model: HierarchicalLSTMBertCRF, batch_insts_ids, name: str,
                              insts: List[Instance]):
    type_dict = {'all': [
        'afcs',
        'shr',
    ...
    for i in range(len(type_dict[config.type])):
        ...
        fscore = 2.0 * precision * recall / (precision + recall) if precision != 0 or recall != 0 else 0
        logging.info(
            "[* %s *] Precision: %.2f, Recall: %.2f, F1: %.2f" % (type_dict[config.type][i], precision, recall, fscore))
    return 0
...
def main():
    ...
    trains_add_devs = reader.read_txt(conf.train_file, conf.train_num, opt.type,
                                      int(conf.max_len) - 12)  # [[inst_sub1,inst_sub2],[],...]  inst:
    ...
    trains = list(
        chain.from_iterable(trains_add_devs[:int(opt.train_dev_split_rate * len(trains_add_devs))]))  # [inst,inst,...]
    devs = list(
        chain.from_iterable(trains_add_devs[int(opt.train_dev_split_rate * len(trains_add_devs)):]))  # [inst,inst,...]
    ...
    model = HierarchicalLSTMBertCRF.from_pretrained(conf.bert_model_dir, config=cfig)  # 加载原始Roberta-wwm权重
    model.to(cfig.device)
...
def main_predict():
    ...
    for i in tqdm(merge_lst):
        ...
        sub_data.write('\n')
    test_file = r"/root/autodl-tmp/CCKS-2021-Financial-Event-Extraction_Rank-6th-main/output.txt"
    prediction_file = r"/root/autodl-tmp/CCKS-2021-Financial-Event-Extraction_Rank-6th-main/saved_model_roberta_split_train_data_base/系统之神与我同在_valid_result.txt"
    evaluate_test_set(test_file, prediction_file)
...
def eval_err_output():
    ...
    for suffix in ['x']:
        ...
        logging.info("\n")
        logging.info("Loading the datasets...")
        trains_add_devs = list(
            chain.from_iterable(reader.read_txt(conf.train_file, conf.train_num, opt.type, int(conf.max_len) - 12)))
        # all_tests = reader.read_test_txt(conf.test_file, conf.train_num)
        # for idx in range(len(all_tests)):
        #     # 给对象打分类标签
        #     if all_tests[idx].id in cls_dict.keys():
